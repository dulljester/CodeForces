...
int n;
char s[N],which[1<<21];
cell pool[N<<1],*ptr = pool,*sink,*root,*q[N<<1],**head,**tail;
...
cell *init( int len, int pos ) {
	cell *x = ptr++;
	x->count = x->freq = x->u = x->mask = 0, x->pos = pos, x->len = len, x->slink = NULL;
	return x;
}
...
cell *split( cell *x, int ch, int pos ) {
	...
	for ( add_son(x,ch,z), x = x->slink; x && TST(x->u,ch) && x->son[ch] == y && !is_primary(x,y); add_son(x,ch,z), x = x->slink ) ;
