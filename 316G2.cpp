/*
 * G3. Good Substrings
 * TOPIC: generalized suffix automaton
 * status: Accepted
 */
#include <map>
#include <assert.h>
...
#define TST(x,ch) ((x) && (x)->son.find(ch) != (x)->son.end())
#define Q (3*N)
#define A 26
#define L(k) ((k)&((~(k))+1ULL))
...
cell *init( int len, int pos ) {
	cell *x = ptr++;
	x->len = len, x->count = 0, x->slink = NULL, x->pos = pos, x->son.clear();
...
cell *update( int ch, int pos, int idx ) {
	...
	for ( ++new_sink->freq[idx], add_son(x=sink,ch,new_sink); (x=x->slink) && !TST(x,ch); add_son(x,ch,new_sink) ) ;
