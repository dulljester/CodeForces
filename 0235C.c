	...
	for ( ;1 == scanf("%s",s+1); ) {
		...
		for ( scanf("%d",&qr); qr-- && 1 == scanf("%s",s+1); ) {
			...
			for ( ans = 0, x = root, k = 0, i = 1; i <= (m+m); ++i ) {
				/*printf("2*m = %d, i = %d\n",m+m,i);*/
				assert( x && x->len >= k && (!x->slink || x->slink->len < k) );
				...
				if ( x->len < k ) k = x->len;
				x = x->son[ch], ++k;
				if ( !x->slink || x->slink->len < k ) {
					if ( k >= m ) ans += x->freq;
					continue ;
				}
				for ( safe = 0, unsafe = n; safe+1 < unsafe; ) {
					y = ancestor(x,mid=(safe+unsafe)/2);
					if ( !y || y->len < k ) unsafe = mid;
					else safe = mid;
				}
				assert(x = ancestor(x,safe));
				if ( k >= m ) ans += x->freq;
			}
			printf("%I64d\n",ans);
		}
